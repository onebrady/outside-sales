...
const IMAGE_FIELD_KEY = "sec43a9b5c";
const CATEGORY_FIELD_KEY = "sd23aef5d0";
const VIN_FIELD_KEY = "s5a42c40e3";
const MANUFACTURER_FIELD_KEY = "s5249431e2";
const MODEL_FIELD_KEY = "s53cc6f27b";
const PRICE_FIELD_KEY = "s042a955ed";
const DAYS_FLOORED_FIELD_KEY = "s1b1cec23a";
const LOCATION_FIELD_KEY = "s7c49ff50c";
...
function getDisplayTitle(record: SmartSuiteTrailerRecord, vin: string | null): {
  ...
  const alt = rawTitle || vin || "Coming soon trailer";
  ...
  useEffect(() => {
    ...
    async function loadComingSoon() {
      try {
        ...
        const response = await fetch(`${basePath}/api/inventory/coming-soon`);
        ...
        if (!response.ok) {
          ...
          throw new Error(message ?? `Failed to load inventory (status ${response.status})`);
        }
        ...
        if (process.env.NODE_ENV === "development") {
          ...
          console.debug("Fetched coming soon payload", sampleItems);
          console.debug(
            "Sample image field",
  ...
  if (records.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-sm text-slate-500 mt-1">Check back later for new production inventory</p>
      </div>
    );
  }
