...
function getDisplayTitle(record: SmartSuiteTrailerRecord, vin: string | null): {
  ...
  const alt = rawTitle || vin || "In-stock trailer";
  ...
  useEffect(() => {
    ...
    async function loadInStock() {
      try {
        ...
        const response = await fetch(`${basePath}/api/inventory/in-stock`);
        ...
        if (process.env.NODE_ENV === "development") {
          ...
          console.debug("Fetched in-stock payload", sampleItems);
          console.debug(
            "Sample image field",
